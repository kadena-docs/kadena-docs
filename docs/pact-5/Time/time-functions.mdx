...
You can use the following template specifiers are introduced by a single percentage (`%`) character to format time strings:
...
When higher resolution is desired, you can explicitly format times with `%v` template and related codes.
