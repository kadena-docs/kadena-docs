...
The initial (`real-world-asset-v1`) implementation of the Kadena RWA token standard focuses on the functions required for an issuing **agent** or **owner** to bring assets on-chain. 
The implementation includes functions and capabilities for:
- Assigning participant roles as token issuing agents, token owners, and token investors.
...
For the initial implementation of the Kadena RWA token standard, the identity registry only supports registering and deleting identities without performing identity verification.
Because of this simplified implementation, the Kadena RWA token standard implements the identity registry and the identity storage features in a single module. 
The functions defined in the ERC-3643 Identity Registry Storage interface are implemented in the Kadena RWA `identity-registry-v1` interface.
