...
- Peer-to-peer connections using [`peer` endpoints](/api/peer-to-peer/api-get-cut-peers)
- Node configuration using the [`config` endpoint](/api/peer-to-peer/api-config)
...
- The `/cut/peer` portion of the peer-to-peer network is responsible for communicating the consensus state across a set of distributed network nodes and all of the chains in the network.
